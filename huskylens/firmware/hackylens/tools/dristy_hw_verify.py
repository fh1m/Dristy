...
HACKYLENS = Path(__file__).resolve().parents[1]
HUSKY = HACKYLENS.parents[1]
IMAGE = HACKYLENS / "dist" / "hackylens-full-huskylens-sen0305.bin"
DOCTOR = HUSKY / "tools" / "dristy_doctor.py"
HKFLASH = HACKYLENS / "tools" / "hkflash.py"
ENV_SH = HACKYLENS / "env.sh"
...
def main() -> int:
    ...
    if not args.skip_build:
        build = run(
            ["bash", "-lc", f"source {ENV_SH} && python tools/build_firmware.py full --board huskylens-sen0305"],
            HACKYLENS,
            timeout=600,
        )
    ...
    if not args.skip_flash:
        flash = run(
            ["sudo", "python", str(HKFLASH), "flash", str(IMAGE),
             "--board", "huskylens-sen0305", "--port", args.port],
            HACKYLENS,
            timeout=300,
        )
    ...
    mon = run(
        ["sudo", "python", str(HKFLASH), "monitor", "--board", "huskylens-sen0305",
    ...
    shot = run(
        ["sudo", "python", str(HKFLASH), "screenshot", "--board", "huskylens-sen0305",
