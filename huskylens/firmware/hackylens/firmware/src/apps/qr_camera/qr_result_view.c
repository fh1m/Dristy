...
{
    hk_ui_display_fill_rect(0, QR_RESULT_STATUS_Y, HK_DISPLAY_REQUIRED_WIDTH, HACKYLENS_FONT_H,
                            DRISTY_COLOR_BG);
    if(status && status[0])
...
void qr_result_view_render(const char *payload, uint16_t scroll_line, uint16_t max_scroll)
{
    ...
    if(!payload[0])
    {
        hk_ui_display_draw_text_centered(QR_RESULT_TEXT_Y + HACKYLENS_FONT_H, "Empty payload",
