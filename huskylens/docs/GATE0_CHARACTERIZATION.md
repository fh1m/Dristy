...
| Property | Value |
|---|---|
| Board | DFRobot HUSKYLENS, standard (not PRO) |
...
the "HUSKYLENS doesn't respond" reports on DFRobot's forums, since a naive
...
`vendor/HUSKYLENSUploader` holds 11 `.kfpkg` packages (V0.3.3 through
...
`vendor/huskylens-isp-stub/isp_prog_huskylens.bin` provides the display-aware
second-stage ISP.
