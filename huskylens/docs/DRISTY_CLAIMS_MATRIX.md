...
**Baseline (pre-fix, USB `/dev/ttyUSB0`, HackyLens 0.4.0 + Dristy menu only):**
...
| Stock 0x20–0x3E | ARCH §10 | `dristy_husky_compat` | `hl_probe` | HW-05 knock |
| HK `H`/`K` frames | HackyLens | `external_link_service` | HK tools | HW-03 HKHELP |
...
```text
# After flash (SEN0305):
pip install -e "huskylens/dristy-py[viewer]"
dristy-viewer --port /dev/ttyUSB0 --mode detect_track

# Edit firmware / modes → rebuild → flash → same viewer
python huskylens/firmware/hackylens/tools/build_firmware.py full --board huskylens-sen0305
sudo python huskylens/firmware/hackylens/tools/hkflash.py flash dist/... --board huskylens-sen0305 --port /dev/ttyUSB0
```
...
| Dristy boot LCD (no HackyLens 1bpp) | UI plan | `display_binding` → `dristy_boot_view_show` | — | HW-04 |
