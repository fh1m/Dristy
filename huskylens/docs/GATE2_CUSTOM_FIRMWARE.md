...
Upstream HackyLens builds on Windows only. Three portability changes were
...
refuses paths outside the invocation prefix. Upstream assumes the HackyLens
tree *is* the repository root, which holds for a standalone clone. When
HackyLens sits inside a larger repository, the computed path is wrong and git
prints:
