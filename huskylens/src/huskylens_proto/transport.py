"""Serial transport for the HUSKYLENS.
...
#: USB vendor IDs used by the USB-UART bridges found on HUSKYLENS boards.
...
def find_ports() -> list[str]:
    """Return device paths for every attached HUSKYLENS-compatible bridge."""
    return [p.device for p in list_ports.comports() if p.vid in KNOWN_VIDS]


class SerialTransport:
    """A framed, resynchronising serial link to the HUSKYLENS."""
