#!/usr/bin/env python3
"""Bring up the SEN0305 link and report what the device is.
...
from dlp_proto import (  # noqa: E402
    CANDIDATE_BAUDS,
    Command,
    DlpClient,
    SerialTransport,
...
from dlp_proto.client import ProtocolError  # noqa: E402
...
def describe(device: Dristy) -> dict:
    report: dict = {}
...
def main() -> int:
    ...
    if port is None:
        detected = find_ports()
        if not detected:
            print("No SEN0305-compatible USB-UART bridge found.", file=sys.stderr)
            return 2
        port = detected[0]
    ...
    if not live:
        print(
            ...
            "I2C. On the SEN0305, open General Settings -> Protocol Type and\n"
        ...
        with SerialTransport(port=port, baudrate=baud) as transport:
            report["device"] = describe(Dristy(transport))
