#!/usr/bin/env python3
"""Stage and build the Dristy K210 ISP stub."""
...
def find_toolchain() -> Path | None:
    ...
    candidates.extend([
        ...
        WORKSPACE / "dristy" / "_deps" / "kendryte-toolchain" / "bin",
        WORKSPACE / "dristy-legacy" / "_deps" / "kendryte-toolchain" / "bin",
    ])
...
def main() -> int:
    ...
    output_bin = output / f"isp_prog_dristy{suffix}.bin"
    ...
    if not args.headless and not args.lcd_init_only:
        release_bin = ROOT / "isp_prog_dristy.bin"
