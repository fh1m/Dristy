...
- `sen0305` is the runtime-qualified, releaseable port using the
  `dristy-full` runtime profile.
...
- Application time needs use the public `dristy.cap.time` 0.1 capability;
  runtime and MicroPython button reads share `dristy.cap.input` 0.1; and
  settings, camera sessions, Sleep, and MicroPython light writes share
  `dristy.cap.lights` 0.1. The native external-link service and MicroPython
  UART/I2C bindings share `dristy.cap.external-link` 0.1. Private
