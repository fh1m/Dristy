---
contract-id: dristy.architecture-vision
...
Текущая Dristy v0.4 является reference implementation и technology
...
Dristy создаётся не только как альтернативная прошивка для одного устройства.
...
Dristy SEN0305 является первой платой, на которой развивается и проверяется
...
> Dristy develops a candidate open application architecture for programmable
...
python tools/run_project.py color-tracker --board sen0305
python tools/new_app.py color-tracker --from-project projects/color-tracker
python tools/build_firmware.py full --board sen0305 --enable-app color-tracker
```
