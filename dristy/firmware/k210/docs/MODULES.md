...
`firmware/include/dristy/capability/` contains the experimental Capability
...
- `dristy/capability/common.h`, `inventory.h`, and `owner.h` for the public
...
- `apps/face_detect/face_detect_app.h` is the sole public FACE DETECT interface; its private YOLO decoder, controller, DVP adapter, view, configuration, and types remain inside the module. The KPU model is read from `/dristy.kmodels/detect.kmodel` through the shared AI runtime.
...
- `apps/object_detect/object_detect_app.h` is the sole public OBJECT DETECT interface. The module owns VOC20 decoding, class labels, overlays, settings, persistence, and diagnostics. Its SD package is `/dristy.kmodels/object20/`.
