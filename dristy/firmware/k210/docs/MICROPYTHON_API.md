---
contract-id: dristy.micropython-api
...
Dristy embeds upstream MicroPython inside the main firmware. A program runs
...
| API | Result |
|---|---|
| `buttons()` | current debounced button bit mask from `dristy.cap.input` |
...
UART and I2C use the descriptor-selected SEN0305 external connector routes.
The script claims that connector on its first UART or I2C call; the normal
Dristy external-link service is suspended until script cleanup.
...
After normal or cooperative execution, Dristy deinitializes the VM, restores lights and the
