# MicroPython внутри Dristy: исследование и план реализации

Дата исследования: 2026-08-04. Фактические результаты обновлены 2026-08-09.

## Краткий вывод

Пункт 5 roadmap реализуем на текущем K210 и в текущей архитектуре Dristy.
...
остаться на core 0. Аппаратные Python-bindings вызывают сервисы Dristy через
...
`firmware/src/services/core1_executor.c`, `build/sdk-full/dristy_full` и
...
deinit. Это ровно нужная модель владения: Dristy остаётся основной
...
- без стандартного `machine`: вместо него модуль `dristy` с контролируемыми
  объектами;
...
ветку. Перенос всего port вернёт в Dristy вторую HAL, второе владение
...
| --- | ---: | --- |
| `0x000000..image_end` | переменный | Dristy firmware |
...
- LEGO API docs и анализатор, заменив их stubs/docs модуля `dristy`;
- run/download flow, заменив одним Dristy Web Serial client.
...
и связались с Dristy. Минимальная VM добавила 135038 байт text, 8 байт data и
...
- Dristy Code выделен в отдельный repository `dristy-code` с полным
  buildable source tree на проверенном Pybricks Code pin и материализованными
  Dristy transport/UI/tests. Upstream MIT license, attribution и pin сохранены;
...
on-device приложение, HMPY v1, Python CLI и отдельный Dristy Code repository
...
`Dristy K210`, capabilities `511`; на живой камере прошёл stress из 100
