# Dristy Platform Roadmap

## Назначение

Этот roadmap переводит Dristy из firmware-specific проекта в candidate open
...
Dristy v0.4 классифицируется как:
...
- `sen0305` не используется как implicit global assumption.
...
- `sen0305` проходит feature-disabled и full cross-build;
...
```python
from dristy import app, camera, display, input, vision, kpu, storage
```
...
Восстановить полезные функции оригинальной SEN0305, не встраивая legacy policy
в shared platform.
