# Dristy K210 firmware tree

The **product README** (live LCD shots, first-principles explainer) lives at
the repository root: [`../../../README.md`](../../../README.md).

This directory is the board firmware: build, flash, SD models, capability
specs. Development target: DFRobot SEN0305 (OV2640 + ST7789 + K210).

> [!IMPORTANT]
> Dristy v0.4 is a layered K210 reference firmware and MicroPython technology
...
Development and hardware testing were performed on the [DFRobot SEN0305 SEN0305](https://www.dfrobot.com/product-1922.html?srsltid=AfmBOopFLFOvPDHc_IyIEzhXPL2jOfHyxDgTjD5jBq53ne3zEmhpjHFF). Other SEN0305 revisions may differ, so verify the board, flash layout, and pin mapping before using this firmware on another revision.
...
a FAT32 card so `dristy.kmodels` is at the root. It includes the FACE model
...
python tools\build_firmware.py full --board sen0305
```
...
`dist\dristy-full-sen0305.json`, and
`dist\dristy-full-sen0305.attestation.json`. The attestation binds
...
the `dist\release\dristy-sen0305-v<version>.*` stem and include a
...
```powershell
python tools\build_firmware.py full --board sen0305 --disable-app pong --disable-app terminal
```

### Dristy Code web IDE

The IDE now lives in the separate sibling repository `dristy-code` (normally
checked out at `..\dristy-code`). It contains the complete buildable source
tree derived from MIT-licensed Pybricks Code at pinned commit
`ea9af98d1fd0a842ed76d3a7f83767f363bc1b17`, plus the Dristy Web Serial
transport, UI and tests:

```powershell
Set-Location ..\dristy-code
...
- `isp_stub/isp_prog_dristy.bin` — the bundled display-aware K210 ISP stub used by `hkflash.py` by default.
...
```powershell
python tools\hkflash.py flash-monitor dist\dristy-full-sen0305.bin --board sen0305 --port COM10
```

Capture the current LCD contents without a camera or screen-grabber:

```powershell
python tools\hkflash.py screenshot --board sen0305 --port COM10 --output screen.bmp
```

Debug commands can also open firmware screens directly; for example:

```powershell
python tools\hkflash.py cmd HKSETTINGS --board sen0305 --port COM10
```
...
Dristy Code is intentionally maintained in the separate sibling
`dristy-code` repository.
...
Dristy is released under the [MIT License](LICENSE).
