...
live in `sdcard/dristy.kmodels/`; generated build artifacts stay under
ignored `dist/`.

`tools/ai_model.py` supports five main workflows:

- `inspect` identifies native KModel v3, word-swapped images, nncase
  containers, and the original SEN0305 legacy KPU object model;
...
`sdcard/dristy.kmodels/detect.kmodel` is pinned to Kendryte's standalone
...
in `sdcard/dristy.kmodels/detect.UPSTREAM.txt`.

The similarly named 386,608-byte asset extracted from the original SEN0305
...
python tools\ai_model.py fetch `
  ...
  --out-dir sdcard\dristy.kmodels\object20

python tools\ai_model.py verify sdcard\dristy.kmodels\object20 `
  --spec models\object_detect_voc20.json
```
