#!/usr/bin/env python3
"""Build Dristy firmware through Kendryte standalone SDK."""
...
LEGACY_DEPS = WORKSPACE / "dristy-legacy" / "_deps"
...
def find_micropython() -> Path | None:
    candidates: list[Path] = []
    if os.environ.get("DRISTY_MICROPYTHON_DIR"):
        candidates.append(Path(os.environ["DRISTY_MICROPYTHON_DIR"]))
    for root in dep_roots():
...
def find_littlefs() -> Path | None:
    candidates: list[Path] = []
    if os.environ.get("DRISTY_LITTLEFS_DIR"):
        candidates.append(Path(os.environ["DRISTY_LITTLEFS_DIR"]))
    for root in dep_roots():
...
def write_project_cmake(stage: Path, package: Path | None, board: Board) -> None:
    lines = [
        ...
        "set(_DRISTY_CAPABILITY_LEAF \"${CMAKE_CURRENT_LIST_DIR}/firmware/include/dristy/capability\")",
        "function(_dristy_prune_capability_leaf directory)",
        ...
        "        if(_includes)",
        "            list(REMOVE_ITEM _includes \"${_DRISTY_CAPABILITY_LEAF}\")",
        ...
        "        _dristy_prune_capability_leaf(\"${_subdirectory}\")",
        "    endforeach()",
        "endfunction()",
        "_dristy_prune_capability_leaf(\"${SDK_ROOT}\")",
    ...
    if package is not None:
        ...
        lines.append("set(DRISTY_MICROPYTHON_SOURCES")
        lines.extend(f'    "{cmake_path(path)}"' for path in sources)
        lines.extend([
            ")",
            "target_sources(${PROJECT_NAME} PRIVATE ${DRISTY_MICROPYTHON_SOURCES})",
            "target_include_directories(${PROJECT_NAME} PRIVATE",
...
def write_reproducible_path_map(path: Path, sdk: Path) -> None:
    ...
    # SDK-specific mapping last.  Both layouts then produce /dristy/sdk.
    roots = [
        (ROOT.resolve(), "/dristy/workspace"),
        (sdk.resolve(), "/dristy/sdk"),
    ]
    seen: set[str] = set()
    flags: list[str] = []
...
def write_config(stage: Path, disabled_apps: set[str],
    ...
    lines = [
        "#ifndef HK_CONFIG_H",
        "#define HK_CONFIG_H",
        f'#define DRISTY_FW_VERSION "{version}"',
...
def build_target(name: str, board: Board, sdk: Path, toolchain_bin: Path,
    ...
    path_map = build_dir / "dristy-path-map.cmake"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Dristy full firmware")
    ...
    parser.add_argument(
        "--disable-dristy",
        action="store_true",
        help="Omit Dristy runtime from firmware (Dristy-only build)",
    )
    parser.add_argument(
        "--wdt-fault-injection",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    sdk = find_sdk()
    if not sdk:
        print("[ERR] Kendryte SDK not found. Run: python dristy\\tools\\bootstrap_deps.py", file=sys.stderr)
        return 1
