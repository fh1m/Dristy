#!/usr/bin/env python3
"""Create a small metadata sidecar for Dristy firmware images."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Prepare a Dristy firmware image artifact")
    ...
    try:
        out_name = validate_output_name(
            args.name or f"dristy-full-{board.id}.bin"
        )
    except ValueError as exc:
