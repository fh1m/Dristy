...
REQUIRED_APP_BY_CAPABILITY = {
    "dristy.cap.time": "pong",
    "dristy.cap.input": "pong",
    "dristy.cap.display": "pong",
    "dristy.cap.external-link": "micropython",
    "dristy.cap.lights": "settings",
}
SOURCE_PATHS = (
...
def artifact_paths(board_id: str = BOARD_ID) -> dict[str, Path]:
    ...
    return {
        "image": root / "dristy-full.bin",
        "elf": root / "sdk-full" / "dristy_full",
        "composition": root / "composition.json",
        "capabilities": root / "capabilities.json",
        "attestation": root / "dristy-full.attestation.json",
    }
...
def expected_diagnostic_compositions() -> dict[str, dict[str, Any]]:
    ...
    external = expected["dristy.cap.external-link"]
    fallback_names = {
...
def capture_diagnostic(
    ...
    if attestation.get("build_profile") != "dristy-feature-modified":
...
def latency_projection(contract_receipt: dict[str, Any]) -> dict[str, Any]:
    sources = {
        "input": (ROOT / "firmware/include/dristy/capability/input.h").read_text(
            encoding="utf-8"
        ),
        "time": (ROOT / "firmware/include/dristy/capability/time.h").read_text(
            encoding="utf-8"
        ),
