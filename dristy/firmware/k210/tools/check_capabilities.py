...
def validate(root: Path = ROOT) -> list[str]:
    ...
    try:
        ...
        for board_id in ("sen0305", "sipeed-maix-cube"):
        ...
        time = next(item for item in catalog if item.id == "dristy.cap.time")
        ...
        input_capability = next(
            item for item in catalog if item.id == "dristy.cap.input"
        )
        ...
        display_capability = next(
            item for item in catalog if item.id == "dristy.cap.display"
        )
        ...
        if [item.id for item in composed_by_board["sen0305"].capabilities] != [
            "dristy.cap.time", "dristy.cap.input",
            "dristy.cap.display", "dristy.cap.external-link",
            "dristy.cap.lights"
        ]:
            failures.append(
