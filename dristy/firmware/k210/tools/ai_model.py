#!/usr/bin/env python3
"""Inspect, convert, package, and verify K210 AI models for Dristy."""
...
def bootstrap_toolchain(output_dir: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="dristy-nncase-") as temp:
...
def inspect_bytes(data: bytes) -> dict[str, Any]:
    ...
    if data[:4] == bytes.fromhex("12345678"):
        ...
        if hashlib.sha256(data).hexdigest() == HUSKY_OBJECT_SHA256:
            result.update({
                "profile": "dristy-object-detect-voc20",
                "input_bytes": 230400,
...
def fetch_and_package(spec_path: Path, output_dir: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="dristy-model-") as temp:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "inspect":
            ...
            with tempfile.TemporaryDirectory(prefix="dristy-convert-") as temp:
