# Dristy ISP stub

`isp_prog_dristy.bin` is bundled from
[`aztechell/dristy-isp-stub`](https://github.com/aztechell/dristy-isp-stub).

- Release: `v1.1`
- Source commit: `2b86fb85de22467c52286735db8ad656e7f7b8a1`
- Size: 17,856 bytes
- SHA-256: `da6305613ff9179afd439be1227ec877d583cde351afed604c7e053052f57cd7`
- License: Apache-2.0, reproduced in `LICENSE`

The protocol, SLIP implementation, and flash driver derive from
`loboris/ktool` commit `0345aa90d9b3830641373fb4e3ce4edf45d0a46f`.
The pinned Kendryte SDK baseline is
`02576ba67e8797444f3ee3f34c625b5ed048e707`.
