...
class BoardDescriptorTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.runtime = board_contract.load_board("sen0305")
    ...
    def test_runtime_and_conformance_invariants(self) -> None:
        self.assertEqual(self.runtime.runtime_profile, "dristy-full")
        ...
        conformance_profile["runtime_profile"] = "dristy-full"
...
class BoardCompositionAndCliTests(unittest.TestCase):
    ...
    def test_flash_defaults_come_from_programming_descriptor(self) -> None:
        ...
        self.assertEqual(args.reset_profile, "dristy-uploader")
        self.assertIs(
    ...
    def test_staging_contains_exactly_one_selected_bsp(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(boards, ["sen0305"])
    ...
    def test_no_board_id_behavior_conditionals_exist(self) -> None:
        ...
        adversarial = {
            ...
            "match-case": (
                "match args.board:\n"
                "  case 'sen0305': reset_special()\n"
            ),
            "behavior-table": (
                "RESET_HANDLERS = {\n"
                "  'sen0305': reset_special,\n"
...
class ArtifactAndFlashSafetyTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.board = board_contract.load_board("sen0305")
    ...
    def test_build_attestation_schema_is_exact_canonical_and_hash_bound(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(document["build_profile"], "dristy-full")
    ...
    def test_board_qualified_image_and_sidecar_identity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            output = temporary / "dist" / f"dristy-full-{self.board.id}.bin"
            ...
            self.assertEqual(metadata["build_profile"], "dristy-full")
    ...
    def test_image_qualification_rejects_arbitrary_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            feature.write_bytes(b"0.3.0 sen0305 fake markers do not qualify")
            ...
            feature_result = subprocess.run(
                [
                    ...
                    "--board", self.board.id,
                    "--name", f"dristy-full-{self.board.id}.bin",
    ...
    def test_package_to_flasher_sidecar_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            temporary = Path(directory)
            source = temporary / f"dristy-full-{self.board.id}.bin"
            ...
            stem = f"dristy-{self.board.id}-v0.4.0"
...
class ResourceEvidenceTests(unittest.TestCase):
    ...
    def test_build_path_mapping_is_global_and_host_paths_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("/dristy/sdk", text)
            self.assertIn("/dristy/workspace", text)
            self.assertLess(
                text.index("/dristy/workspace"),
                text.index("/dristy/sdk"),
                "the specific SDK prefix map must follow the broad workspace map",
            )

            safe = temporary / "safe.bin"
            safe.write_bytes(b"/dristy/workspace/firmware/main.c")
    ...
    def test_phase1_hardware_evidence_is_canonical_sanitized_and_hash_bound(self) -> None:
        ...
        self.assertEqual(document["board"], "sen0305")
