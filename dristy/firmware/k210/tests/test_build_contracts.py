...
HKFLASH_SPEC = importlib.util.spec_from_file_location(
    "dristy_hkflash", ROOT / "tools" / "hkflash.py"
)
...
class BuildContractsTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.board = BUILD_FIRMWARE.load_board("sen0305")
    ...
    def test_generated_config_uses_canonical_version(self):
        ...
        self.assertIn(f'#define DRISTY_FW_VERSION "{expected}"', config)
    ...
    def test_wdt_fault_injection_is_explicit_and_test_build_only(self):
        ...
        self.assertIn(f'#define DRISTY_FW_VERSION "{expected}-wdtfi"', config)
