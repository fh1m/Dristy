...
static uint8_t g_blank_glyph[DRISTY_FONT_H * DRISTY_FONT_ROW_BYTES];
                                ...
                                uint16_t foreground, uint16_t background)
{
    ...
    (void)background;
    g_operation++;
    check(x + glyphs * DRISTY_FONT_W <= 320U,
          "text stays within display width");
    check((uint32_t)y + DRISTY_FONT_H <= 240U,
                                      ...
                                      uint16_t background)
{
    (void)foreground;
    (void)background;
    check(strlen(text) * DRISTY_FONT_W <= 320U,
