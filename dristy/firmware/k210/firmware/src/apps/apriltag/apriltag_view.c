                                   ...
                                   uint8_t count)
{
    ...
    for(uint8_t i = 0; i < count; i++)
    {
        ...
        label_width = (uint16_t)(strlen(label) * DRISTY_FONT_W);
        label_x = results[i].center_x > (int16_t)(label_width / 2U) ?
            (uint16_t)(results[i].center_x - label_width / 2U) : 0U;
        label_y = results[i].center_y > (int16_t)(DRISTY_FONT_H / 2U) ?
            (uint16_t)(results[i].center_y - DRISTY_FONT_H / 2U) : 0U;
        if(label_x + label_width > width)
            label_x = width > label_width ? width - label_width : 0U;
        if(label_y + DRISTY_FONT_H > height)
            label_y = height > DRISTY_FONT_H ? height - DRISTY_FONT_H : 0U;
