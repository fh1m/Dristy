...
static void camera_view_compose_overlay(hk_ui_display_surface_t *surface,
                                        ...
                                        uint16_t box_w)
{
    const uint16_t box_h = DRISTY_FONT_H + 4U;
    ...
    for(uint16_t index = 0; text[index] && text_x + (index + 1U) * DRISTY_FONT_W <= box_x + clipped_w; index++)
    {
        const uint8_t *glyph = hk_font_glyph(text[index]);
        uint16_t glyph_x0 = text_x + index * DRISTY_FONT_W;

        for(uint16_t y = 0; y < DRISTY_FONT_H && text_y + y < surface->height; y++)
        {
            for(uint16_t x = 0; x < DRISTY_FONT_W; x++)
            {
                uint8_t bit = glyph[y * DRISTY_FONT_ROW_BYTES + x / 8U] & (0x80U >> (x & 7U));
                if(bit)
                {
...
static uint16_t camera_view_overlay_width(const char *text)
{
    ...
    if(!text)
        return 0;
    width = (uint32_t)strlen(text) * DRISTY_FONT_W + 8U;
    if(width > max_width)
        width = max_width;
                                 ...
                                 uint16_t bg)
{
    if(!present || !text)
        return;
    while(*text && x + DRISTY_FONT_W <= present->width &&
          y + DRISTY_FONT_H <= present->height)
    {
        const uint8_t *glyph = hk_font_glyph((uint8_t)*text++);

        for(uint16_t glyph_y = 0; glyph_y < DRISTY_FONT_H; glyph_y++)
        {
            uint8_t *dst = present->rgb565_be +
                (uint32_t)(y + glyph_y) * present->stride_bytes + (uint32_t)x * 2U;
            for(uint16_t glyph_x = 0; glyph_x < DRISTY_FONT_W; glyph_x++)
            {
                uint8_t bit = glyph[glyph_y * DRISTY_FONT_ROW_BYTES + glyph_x / 8U] &
