...
Flashing a SEN0305 has always been a one-way door. The stock kflash
second-stage ISP is write-only: it implements `D4` write, `D7` init, `D8` erase
and `D9` erase status, but has **no read command**. Dristy' own hardware
...
public SEN0305 flash-dumping tool could be found.
...
[dristy-isp-stub](https://github.com/aztechell/dristy-isp-stub) with two
...
8 MiB revision that Dristy defends against did not appear here.
...
1. **Byte-exact**, from `stock_flash_dump1.bin` — including DFRobot's
   unpublished model region. This did not previously exist for any SEN0305.
2. From the mirrored official `.kfpkg` packages in `vendor/SEN0305Uploader`
   (11 packages, checksums in `artifacts/firmware_checksums.txt`).
3. Via the Dristy Uploader's "Reset Memory" function on Windows.
...
```bash
cd dristy/firmware/hl-isp-dumper && python3 tools/bootstrap_deps.py && python3 tools/build.py
