...
**Dristy** is vision co-processor firmware for the DFRobot SEN0305 (Kendryte
K210 + OV2640). It replaces the stock camera firmware with a modular pipeline
that exposes detection, classification, tracking, fiducial markers, optical
flow, colour/line following, and a host protocol for flight controllers, ROS 2
...
**Dristy** (দৃষ্টি) — Bengali for "vision" / "sight". Boot splash, protocol
identity, and host-facing names all use this word. The hardware target is the
SEN0305 K210 camera module.
...
### Mailbox frame policy (already implemented in Dristy)
...
Both AprilTag and QR are already vendored in Dristy (`firmware/third_party/`).
...
Dristy already vendors the full `apriltag` library with TAG36H11 support
...
### Pin availability (Dristy SEN0305)
...
Backward-compatible with the stock Dristy protocol for basic queries, but
...
|---|---|---|
| **P0: Skeleton** | Rename to Dristy, boot splash, PLL1 raise, mailbox→KPU wiring | existing Dristy |
...
3. **First pipelined dual-core vision co-processor firmware** on Dristy with
